...
export function PerformanceByYearChart({ player }: { player: string | null }) {
  ...
  return (
    <div className="h-[380px] w-full rounded-xl border border-ink-800 bg-ink-900/50 p-4">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={state.data} margin={{ top: 8, right: 24, bottom: 8, left: 0 }}>
          ...
          <ReferenceLine y={80} stroke="#8256ad" strokeDasharray="4 4" label={{ value: '80%', fill: '#ab86c7', fontSize: 11 }} />
          <Line
            type="monotone"
            dataKey="rendimiento"
            name="Rendimiento"
            stroke="#dd7c4c"
            strokeWidth={2.5}
            dot={{ r: 3, fill: '#0f1712', stroke: '#dd7c4c', strokeWidth: 2 }}
            activeDot={{ r: 5 }}
          />
        </LineChart>
      </ResponsiveContainer>
